...
class Message{
    ...
    {
        $details_array = array();
        $query = mysqli_query($this->con,"SELECT date,body,user_to FROM messages WHERE (user_to='$userLoggedIn' AND user_from='$username') OR (user_from='$userLoggedIn' AND user_to='$username')
    ...
    public function getConvos()
    {
        ...
        while($row=mysqli_fetch_array($query))
        {
            ...
            foreach($convos as $username)
            {
                ...
                //here $split[0] is above split variable whole
                $split = $split[0] . $dots; 
